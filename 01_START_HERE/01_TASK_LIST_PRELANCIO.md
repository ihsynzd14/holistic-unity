...
- [X BUT] **F5 — Password forgot** → email arriva → link cliccabile → nuova password PS> FORGOT PASSWORD SENT EMAIL TO THE SPAM (PREVENT IT, LINK NOT WORKS CHECK)
       🔧 F5 "link non funziona" FIXED lato WEB 2026-06-03 (ISKO). Causa: iOS chiamava `resetPasswordForEmail` SENZA redirectTo + il web usava il flusso PKCE (`/auth/callback`→`exchangeCodeForSession`) che funziona solo se il link si apre nello stesso browser che l'ha richiesto (le app Mail aprono altrove → fallisce) + scanner spam che consumano il token + scadenza 1h. NON (solo) errore utente. Fix: migrato al flusso token_hash/verifyOtp (niente code_verifier → da qualsiasi browser/app/device): nuova route `/auth/confirm` (client + therapist — progetto Supabase condiviso `bqyqkvkzkemiwyqjkbna`, 1 template recovery), `forgot-password` di entrambe → `redirectTo=/auth/confirm?next=/reset-password`, template recovery → `{{ .RedirectTo }}&token_hash={{ .TokenHash }}&type=recovery`, iOS `sendPasswordReset` → `redirectTo=https://app.holisticunity.app/auth/confirm?next=/reset-password`. **DEPLOYATO + PUSHATO + VERIFICATO LIVE 2026-06-03**: route 307 in prod (app + therapistportal), recovery template live contiene `token_hash`, allowlist coperta dai wildcard `/**`. **Web reset LIVE.** Resta: (d) build iOS su Mac (fix già nel codice) + test e2e del reset. Spam = deliverability separata (Brevo/Resend domain auth, già fatto).
...
- [-] Per ognuna delle 36 email sopra: trigger via QA in production con un account di test, verifica:
  ...
  **Audit C11-C15 + T1-T17 + A1-A4 (2026-06-03, ISKO via Claude)** — le 26 email residue tracciate (code-level + live Brevo template check): [03_Security_and_Audits/EMAIL_FLOWS_AUDIT_REMAINING_2026-06-03.md](../03_Security_and_Audits/EMAIL_FLOWS_AUDIT_REMAINING_2026-06-03.md). Risultato: 21 ✅ + 3 ⚠️ wired-ma-template-inattivo + 4 🟥 orphaned + 9 ⬛ missing. **🔴 CRITICO**: i template Brevo 12/13/14 (PAYOUT_SENT, PAYOUT_FAILED, ADMIN_PAYOUT_FAILED) sono `isActive=false` → T13/T14/A4 falliscono silenziosamente in prod. Fix = attivare i template in Brevo (richiede autorizzazione utente). Altri gap pre-lancio bassi: A1 (alert admin nuovo terapista pending), A2 (alert admin nuovo report). Resto post-launch (C7/T9 1h reminder, T12 reschedule client→terapista, T15/C13 review, T16 statement, T17 doc-expiry, C12/C14/C15/T3/A3).
