...
- [X BUT] **F5 — Password forgot** → email arriva → link cliccabile → nuova password PS> FORGOT PASSWORD SENT EMAIL TO THE SPAM (PREVENT IT, LINK NOT WORKS CHECK)
       🔧 F5 "link non funziona" — ROOT CAUSE + FIX CODICE 2026-06-03 (ISKO). Causa: (1) iOS `sendPasswordReset` chiamava `resetPasswordForEmail(email)` **senza redirectTo** → link alla Site URL, mai a /reset-password; (2) il web usava il flusso **PKCE** (`/auth/callback`→`exchangeCodeForSession`) che funziona SOLO se il link si apre nello stesso browser che l'ha richiesto (le app Mail aprono in in-app browser/altro device → fallisce → "link non valido o scaduto"); (3) scanner anti-spam che consumano il token monouso (aggravato dallo spam); (4) scadenza 1h. NON era (solo) errore utente. **Fix (flusso token_hash/verifyOtp, niente code_verifier → funziona da qualsiasi browser/app/device)**: nuova route `/auth/confirm` (`verifyOtp`) in **client-webapp E therapist-webapp** (progetto Supabase condiviso `bqyqkvkzkemiwyqjkbna` → 1 solo template recovery, quindi entrambe le app vanno migrate insieme); `forgot-password` di entrambe ora passa `redirectTo=/auth/confirm?next=/reset-password`; template recovery (`push-email-templates.mjs` `bodyRecovery`) → `{{ .RedirectTo }}&token_hash={{ .TokenHash }}&type=recovery`; iOS `sendPasswordReset` ora passa `redirectTo=https://app.holisticunity.app/auth/confirm?next=/reset-password`. Tutto tsc-clean (client+therapist). **GO-LIVE (ordine obbligatorio)**: (a) deploy Vercel di ENTRAMBE le webapp → /auth/confirm live (verificato 404 in prod prima del deploy); (b) Supabase Dashboard → Auth → URL Configuration → Redirect URLs: aggiungi `https://app.holisticunity.app/auth/confirm` + `https://therapistportal.holisticunity.app/auth/confirm`; (c) push template (`SUPABASE_ACCESS_TOKEN` PAT → `node scripts/email-templates/push-email-templates.mjs`); (d) build iOS su Mac + rilascio (gli utenti con app vecchia non hanno il redirectTo finché non aggiornano — il web reset è fixato al deploy). NB: lo spam è coperto a parte dalla deliverability (Brevo/Resend domain auth 2026-06-03). **Resta da fare: a/b/c/d.**
...
- [HALF WORKS] **F7 — Visualizza profilo terapeuta** → bio, gallery, video, recensioni, servizi visibili ( WHEN WE CLICK TO BOOK BUTTON WE GET CHOOSE SERVICE FOR FIRST TIME BECAUSE  INTIALIZATION MAYBE SO WHEN I CLOSE AND REOPEN I SEE CALENDAR FOR BOOKING WHICH WAS EXPECTED TO SEE SO I NEED FROM YOU TO FIX IT. 2) VIDEO PLAYER NOT WORKS YOUTUBE ERROR 153, VIMEO WORKS FOR EXAMPLE )
       🔧 F7·a (Prenota → "Choose Service" invece del calendario) FIXED 2026-06-03 (ISKO): root cause = 2 dei 3 entry-point passavano `service: nil` → atterravano su "Choose a Service" (step 0). In particolare "**See Full Calendar**" ([TherapistProfileView.swift:1033](../08_Codebases/iOS_App/Holistic%20Unity/Features/TherapistProfile/TherapistProfileView.swift)) prometteva il calendario ma mostrava la scelta servizio; inoltre con 1 solo servizio lo step "Choose a Service" è un tap inutile. NON era staleness SwiftUI (`.sheet(item:)`+UUID ricrea già il VM fresco). Fix: `BookingFlowViewModel.init` ora auto-seleziona quando il terapista ha **esattamente 1 servizio** → step 1 (calendario) da OGNI bottone; "See Full Calendar" rietichettato **"Prenota"**. Con ≥2 servizi la scelta servizio resta (corretta: gli slot dipendono dalla durata). Build iOS su Mac + QA on-device ancora da fare.
       🔧 F7·b (video player YouTube error 153) FIX APPLICATO 2026-06-03 (ISKO): error 150/153 = il proprietario del video ha **disabilitato l'incorporamento** (lato contenuto, non correggibile in-app per quel video) — ecco perché Vimeo funziona e quel YouTube no. Il path inline `VideoThumbnailPreview` ([VideoPlayerViews.swift](../08_Codebases/iOS_App/Holistic%20Unity/DesignSystem/Components/VideoPlayerViews.swift)) riscritto con **(a)** Safari `customUserAgent` (mancava, a differenza del player Shorts che funziona), **(b)** `origin` nei playerVars, **(c)** YouTube **IFrame Player API** con `onError` → se l'embed è bloccato mostra un fallback **"Guarda su YouTube"** che apre il video nell'app/Safari (così un video con embedding off non è più un vicolo cieco). Vimeo invariato. **Da fare**: build su Mac + test su device con (1) un video YouTube incorporabile (deve riprodursi inline) e (2) uno con embedding disabilitato (deve mostrare il fallback). Lungo termine: guidare i terapisti a usare video incorporabili nel setup profilo (F25).
...
- [ ] **F9 — Booking servizio a pagamento** → Stripe PaymentSheet → Apple Pay (su iOS) o carta → conferma → email conferma
       🔧 2026-06-03 (ISKO): flusso toccato dai fix scheduling A (guard anti-doppia-prenotazione a livello DB), B (fuso orario iOS), C (preavviso minimo + buffer iOS) — dettaglio in F27. Code-fix fatto; QA paga→conferma end-to-end ancora da eseguire.
...
- [ ] **F13 — Reschedule client** → terapeuta vede notifica → accetta/rifiuta → email a client
       🔧 2026-06-03 (ISKO): il reschedule ora costruisce l'istante proposto nel fuso del terapista (fix B) e passa per il guard anti-doppia-prenotazione (fix A) — vedi F27. QA da eseguire.
...
- [ ] **F27 — Setup disponibilità**: weekly recurring slots + one-off + blocked dates
       🔧 Scheduling audit + fix 2026-06-03 (ISKO): esaminato l'intero sistema disponibilità→slot→prenotazione, trovati e corretti 4 bug di correttezza. **CODICE FATTO, QA end-to-end su device/prod ANCORA DA FARE** (questo flow + F8/F9/F10/F13). **A doppia-prenotazione**: il trigger `prevent_overlapping_active_bookings` ignorava lo stato `pending_payment` (intero flusso web Stripe Checkout NON protetto) ed era un BEFORE-trigger non a prova di concorrenza → aggiunto vincolo `EXCLUDE USING gist` `bookings_no_overlap` (tutti gli stati live incl. pending_payment) + trigger esteso + `api/checkout/create` ora ritorna 409 su conflitto. Migration `20260603120000_bookings_overlap_exclusion_constraint.sql` **già eseguita in prod via Dashboard** (dopo audit overlap esistenti). **B ora-sbagliata-cross-tz (iOS)**: slot generati nel fuso del terapista ma `scheduled_at` materializzato nel fuso del device → helper unico `TherapistAvailability.resolveSlotInstant` usato in booking+reschedule+add-to-calendar, `booking.timezone` ora = fuso terapista, hint UI quando i fusi differiscono. **C iOS-ignorava-minNotice/buffer**: `SupabaseBookingRepository.getAvailableSlots` ora applica preavviso minimo + buffer tra sessioni (come il web), cadenza slot 30→15min, decode `TherapistAvailability` reso tollerante (default 24/15). **D web/admin-ignorava-eccezioni** (giorno libero / orari speciali): i reader leggevano `type`/`ranges` ma l'editor scrive `isAvailable`/`customRanges` → corretto in 3 reader (client-webapp `lib/booking/slots.ts` + `api/checkout/create`, admin-dashboard `lib/capacity/capacity.ts`) via helper condiviso `resolveDayRanges`; entrambe le web app passano `tsc --noEmit`. **DEPLOY richiesto**: client-webapp + admin-dashboard (Vercel) + iOS (build Mac). Dettaglio completo in `memory/scheduling-architecture.md`.
...
- [-] Per ognuna delle 36 email sopra: trigger via QA in production con un account di test, verifica:
  ...
  **Architectural finding**: l'Edge Function `send-brevo-email` (chiamata dai route handler) è **deployata in prod (v12) ma NON in git** (verificato via `supabase functions list`). Stesso pattern per altre 6 Edge Functions: `sync-brevo-contact`, `send-session-reminders`, `check-dormant-users`, `get-available-slots`, `validate-vat`, `validate-promo`. Operational debt tracciato in Post-launch hardening (`supabase functions download` × 7 + commit).
  ...
  Report completo (architettura email + per-flow trace + Opzioni A/B/C per fix C2 + impact assessment): [03_Security_and_Audits/EMAIL_FLOWS_AUDIT_CLIENT_2026-05-23.md](../03_Security_and_Audits/EMAIL_FLOWS_AUDIT_CLIENT_2026-05-23.md).

  **Audit C11-C15 + T1-T17 + A1-A4 (2026-06-03, ISKO via Claude)** — le 26 email residue tracciate (code-level + live Brevo template check): [03_Security_and_Audits/EMAIL_FLOWS_AUDIT_REMAINING_2026-06-03.md](../03_Security_and_Audits/EMAIL_FLOWS_AUDIT_REMAINING_2026-06-03.md). Risultato: 21 ✅ + 3 ⚠️ wired-ma-template-inattivo + 4 🟥 orphaned + 9 ⬛ missing. **🔴 CRITICO → ✅ FIXED (2026-06-03)**: i template Brevo 11/12/13/14 (SESSION_REMINDER_1H, PAYOUT_SENT, PAYOUT_FAILED, ADMIN_PAYOUT_FAILED) erano `isActive=false` (placeholder "Reserved N") → T13/T14/A4 fallivano silenziosamente in prod. **Attivati via API** (`PUT /v3/smtp/templates/{id}` con `isActive=true` + subject + sender), verificati `active=True`. HTML già brandizzato dal push del 2026-06-03. **A1 + A2 IMPLEMENTATI (2026-06-03, code-complete, pending deploy)** — alert admin via cron (no pg_net/secret-in-SQL, segue il pattern cron esistente):
    - Migration [20260603140000_admin_alert_notified_columns.sql](../08_Codebases/iOS_App/supabase/migrations/20260603140000_admin_alert_notified_columns.sql): aggiunge `admin_notified_at` a `therapist_profiles` + `reports`, backfill rows esistenti a `now()` (no flood al 1° run), partial index, e reset `admin_notified_at=null` in `submit_therapist_profile_for_review()` (re-alert su re-submit).
    - Cron [admin-dashboard /api/cron/admin-alerts/route.ts](../08_Codebases/admin-dashboard/src/app/api/cron/admin-alerts/route.ts) (GET+POST, CRON_SECRET bearer): scansiona pending_review + reports non notificati → Brevo template 15/16 a `ADMIN_ALERT_EMAIL` (default support@) → stampa `admin_notified_at`. Schedule orario in [vercel.json](../08_Codebases/admin-dashboard/vercel.json). `tsc --noEmit` clean.
    - Template Brevo **15** (Admin - New Therapist Pending) + **16** (Admin - New Report) creati, brandizzati ([push-brevo-templates.mjs](../08_Codebases/client-webapp/scripts/email-templates/push-brevo-templates.mjs)) + **attivati** (`isActive=true`). Enum aggiornato in [_shared/brevo.ts](../08_Codebases/iOS_App/supabase/functions/_shared/brevo.ts).
    - **DEPLOY richiesto**: (1) applicare la migration al DB; (2) deploy admin-dashboard su Vercel (cron + route); (3) opz. settare env `ADMIN_ALERT_EMAIL`. Nessun redeploy edge function necessario.
  Resto post-launch (C7/T9 1h reminder cron, T12 reschedule client→terapista, T15/C13 review, T16 statement, T17 doc-expiry, C12/C14/C15/T3/A3). **Bug separato trovato**: `billing-reminders` cron chiama send-brevo-email con `{to,subject,html}` (raw) ma la function richiede `template_id` → probabilmente rotto (tracciato a parte).
- [~] Verifica record DNS: `dig TXT holisticunity.app | grep -E "(spf|dmarc)"` + DKIM `dig TXT brevo._domainkey.holisticunity.app`. Tutti devono passare. Test su https://mail-tester.com → score ≥ 9/10
       **2026-06-03 (ISKO via Claude)**: 3 sistemi di invio chiariti — **Resend** (Supabase Auth SMTP, `no-reply@`, auth/password-reset), **Brevo** (transactional, `support@`), **Google Workspace** (inbound MX). DNS Squarespace: SPF root aggiunto `v=spf1 include:_spf.google.com include:spf.brevo.com ~all` ✅; DKIM Brevo `brevo1/brevo2._domainkey` ✅ (chiavi verificate fino a brevo17/18); DKIM Resend `resend._domainkey` + SPF su `send.` subdomain (amazonses) ✅; DMARC `p=none` ✅. **Root cause spam trovato+risolto**: Brevo firmava come `header.d=sendinblue.com` (DKIM non allineato → DMARC fail) perché il dominio non era "authenticated" nella dashboard Brevo → **attivato domain authentication in Brevo** (utente, 2026-06-03). Template branding ri-pushato: 14 Brevo (`push-brevo-templates.mjs`) + 4 Supabase Auth (`push-email-templates.mjs`). **Resta da confermare**: mail-tester ≥9/10 + `dmarc=pass header.d=holisticunity.app` su un send reale post-fix (Brevo path E Resend path).
