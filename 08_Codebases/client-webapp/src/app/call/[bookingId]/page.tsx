...
export default function CallPage() {
  ...
  // Leave: just disconnect. Marking a booking completed is
  // therapist-only server-side (see /api/bookings/[id]/complete), so
  // this never mutated the booking for a client anyway — but the old
  // single "End Session" button still showed a misleading "Sessione
  // completata" screen as if it had. Leave is the honest default for
  // "I'm stepping out"; the booking is untouched either way.
  ...
  // Rejoin after a deliberate Leave — same tab, re-run the same init
  // flow used on first load (fetch a fresh token, reconnect).
  ...
  // UUID — short-circuiting the payout escrow window.
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <div className="max-w-sm rounded-2xl bg-white/10 p-8 text-center backdrop-blur-sm">
          ...
          <p className="mt-2 text-sm text-white/60">
            This can&apos;t be undone from here. Only do this once the session has actually finished.
          </p>
