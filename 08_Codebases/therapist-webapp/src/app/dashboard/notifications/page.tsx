...
export default function NotificationsPage() {
  ...
  // Map our internal locale to a BCP-47 tag for Intl APIs.
  // Without an explicit locale, `toLocaleDateString(undefined, ...)`
  // falls back to the browser's preferred language, which on most
  // machines returns EN — producing months like "May 8" / "Apr 27"
  // on a page that's otherwise all-Italian. (QA finding 2026-05-15.)
  ...
  function timeAgo(dateStr: string): string {
    ...
    if (hours < 24) {
      // Italian (and English) need singular when n === 1:
      // "1 ora fa" / "1 hour ago" vs "5 ore fa" / "5 hours ago".
      // Previously this was hard-coded to the plural form, producing
      // "1 ore fa" / "1 hours ago" — small but visibly broken.
      const key = hours === 1 ? t.notifications.timeAgo.hourAgoOne : t.notifications.timeAgo.hoursAgoMany;
      return `${hours} ${key}`;
    }
    const days = Math.floor(hours / 24);
    if (days < 7) {
      const key = days === 1 ? t.notifications.timeAgo.dayAgoOne : t.notifications.timeAgo.daysAgoMany;
      return `${days} ${key}`;
    }
    return new Date(dateStr).toLocaleDateString(dateFmtLocale, { day: "numeric", month: "short" });
  }
