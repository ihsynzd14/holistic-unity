...
function LoginForm() {
  ...
  const roleError = searchParams.get("error") === "not_therapist";
  const pendingStatus = searchParams.get("status") === "pending_review";
  const { t } = useI18n();
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (data.user) {
      ...
      if (userData?.role !== "therapist") {
        ...
        setError(t.login.notTherapist);
        setLoading(false);
        return;
      }

      // Gate access on admin approval. The therapist_profiles row is
      // created with approval_status = "pending_review" on registration;
      // only "approved" therapists are allowed into the dashboard.
      ...
      // MFA: if the therapist has opted into 2FA, password sign-in only
      // gives an aal1 session — they need to enter their TOTP code on
      // /verify-mfa to upgrade to aal2 before the dashboard layout will
      // let them through.
      const { data: factors } = await supabase.auth.mfa.listFactors();
      const hasVerifiedFactor = (factors?.totp ?? []).some((f) => f.status === "verified");
      if (hasVerifiedFactor) {
        // Log the password-step sign-in even though MFA hasn't been
        // entered yet — useful for "someone tried to log in to my
        // account" forensics.
    ...
    // Audit: log successful sign-in (no-MFA path)
    fetch("/api/security/log-login", { method: "POST" }).catch(() => {});
