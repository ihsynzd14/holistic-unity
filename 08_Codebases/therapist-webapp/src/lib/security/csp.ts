...
export function buildCsp(
  ...
  //   inline scripts come back without a `nonce=` attribute, the browser
  //   blocks them under nonce-CSP, and React stays stuck on the Suspense
  //   fallback ("Caricamento..."). Reverting to 'unsafe-inline' restores
  //   XSS posture equivalent to the original next.config.ts. The `nonce`
  //   parameter is still threaded through for any downstream consumer.
  void nonce;
  const scriptSrc = [
    "'self'",
    "'unsafe-inline'",
  ...
  return [
    ...
    // Stripe Checkout iframe; everything else denied.
