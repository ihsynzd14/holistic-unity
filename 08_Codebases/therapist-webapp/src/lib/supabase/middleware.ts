...
export async function updateSession(
  ...
  // /register is public so therapists can self-serve onboarding; the
  // account they create is blocked until an admin flips approval_status
  // to "approved" (enforced at login + in the dashboard layout).
  // /forgot-password is public so a locked-out therapist can recover.
  // /reset-password is post-recovery (the user has a short-lived
  // session at that point); the page itself checks for an active
  // session and renders the right state.
  const isPublicRoute =
