...
**Status:** ✅ Production (+ biometric gate, deep-link router, and GDPR erasure pipeline added 2026-04-17)
**Owner:** Marcello

## Purpose

Two separate auth surfaces, and a role-split that mirrors the surfaces one-to-one:

- **iOS app — client-only.** Email/password + Sign in with Apple + Sign in with Google. Sign-up auto-assigns `role=client` (no role picker). Any user that happens to have `role=therapist` (e.g. signed up on the web portal and then installed the app) is shown `TherapistWebAppRedirectView` on launch — the iOS app has no therapist UI at all (no dashboard, no earnings, no booking management from therapist POV).
- **Therapist webapp** — email/password only, rejects `role != therapist` after login.

Admin dashboard uses same Supabase auth, gated by **two-factor check**: `ADMIN_EMAILS` env whitelist **AND** `users.is_admin = true` in DB (via `public.is_admin()` RPC). See `admin-dashboard/src/lib/auth/requireAdmin.ts` — both checks must pass; the env whitelist alone is no longer sufficient. A BEFORE UPDATE trigger (`_guard_user_is_admin_updates`) blocks clients from self-escalating.
...
- `03-client-onboarding.md` (post-signup flow)
