...
- Current time is within the rejoin window (see invariants)
...
2. `Booking.canJoinVideoCall` at `Domain/Models/Booking.swift:80-86` allows any time within the session DAY (midnight to midnight)
...
4. Edge function at `supabase/functions/livekit-token/index.ts`:
   ...
   - Returns signed JWT for the LiveKit room `hu-${SHA256(bookingId + salt).prefix(16)}` (see `Config/LiveKitConfig.swift:10-14`)
...
- Join window: midnight-to-midnight of session day (+3h grace after `endTime` for rejoin)
- Room name is deterministic per booking but salted so bookings IDs aren't enumerable
- Only booking parties (client + therapist of that exact booking) can get a valid token
- `booking.status = completed` only on EXPLICIT end by therapist, NEVER on accidental disconnect
